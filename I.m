...
%                 end
    ...
    for i=1:N_trainchar
        class1_train(:,:,:,i)=squeeze(mean(Data.Epoch(i).Target,2));          % size(class1_train)=[2,28,16,40]
        class2_train(:,:,:,i)=squeeze(mean(Data.Epoch(i).NonTarget,2));       % size(class2_train)=[10,28,16,40]
    end
...
%     for j_trainchar=3:N_trainchar
        ...
        A=[];
        cnt=0;
        for i=1:size(class1_train,2)
            for j=1:size(class1_train,3)
                cnt=cnt+1;
                A(cnt,:,:)=squeeze(class1_train(:,i,j,:));      % size(A)=[448,2,40]
            end
        end
        ...
        for i=1:size(A,2)
            for j=1:j_trainchar
                cnt=cnt+1;
                train1(:,cnt)=squeeze(A(:,i,j));      % size(train1)=[448,80]=[feature,sample]
            end
        end
        ...
        A=[];
        cnt=0;
        for i=1:size(class2_train,2)
            for j=1:size(class2_train,3)
                cnt=cnt+1;
                A(cnt,:,:)=squeeze(class2_train(:,i,j,:));      % size(A)=[448,10,40]
            end
        end
        ...
        for i=1:size(A,2)
            for j=1:j_trainchar
                cnt=cnt+1;
                train2(:,cnt)=squeeze(A(:,i,j));      % size(train2)=[448,400]=[feature,sample]
            end
        end
        ...
        [ label_RC_p,label_Char_p ] = myLDA_testI( test,w_LDA );     % myLDA_test
